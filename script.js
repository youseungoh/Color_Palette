...
function renderAll() {
  ...
  const pInput   = document.getElementById('colorPreviewHexInput');
  const prgb     = document.getElementById('colorPreviewRgb');
  if (swatch) swatch.style.background = hex;
  if (pInput && document.activeElement !== pInput) pInput.value = hex;
...
function updateUIExceptInput() {
  ...
  const pInput2 = document.getElementById('colorPreviewHexInput');
  const prgb2   = document.getElementById('colorPreviewRgb');
  if (swatch2) swatch2.style.background = hex;
  if (pInput2 && document.activeElement !== pInput2) pInput2.value = hex;
...
/* 프리뷰 카드 HEX 입력 */
const previewInput = document.getElementById('colorPreviewHexInput');

previewInput.addEventListener('focus', () => {
  previewInput.value = state.hex;
  previewInput.select();
});

previewInput.addEventListener('input', () => {
  const digits = previewInput.value.replace(/[^0-9A-Fa-f]/g, '');
  if (digits.length === 6) {
    const candidate = '#' + digits.toUpperCase();
    if (isValidHex(candidate)) {
      const rgb = hexToRgb(candidate);
      ...
      addHistory(state.hex);
      updateUIExceptInput();
      /* 우측 패널 hex input도 동기화 */
      const hi = document.getElementById('hexInput');
      if (document.activeElement !== hi) hi.value = state.hex;
    }
  }
});
...
previewInput.addEventListener('keydown', e => {
  if (e.key === 'Enter') previewInput.blur();
});
